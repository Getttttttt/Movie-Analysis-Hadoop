...
public class MovieRatingsCountMapReduce {

    public static class RatingsMapper extends TableMapper<Text, IntWritable> {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Count Movie Ratings");
        job.setJarByClass(MovieRatingsCountMapReduce.class);
        ...
        Path outputPath = new Path(hdfsUri + "/usr/local/hadoop/movieratings/ratingsCountOutput");
        ...
        if (completion) {
            // 读取并打印HDFS输出
            printOutputFromHDFS(hdfsUri + "/usr/local/hadoop/movieratings/ratingsCountOutput", conf);
        } else {
            System.exit(1);
        }
